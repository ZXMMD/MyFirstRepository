...
#define COLOR_BADFOOD       RGB(0,0,0)
#define COLOR_GOODFOOD      RGB(255,0,255)
...
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	HPEN hPenBoundary;
	HPEN hOldPen, hOldPen2, hOldPen3;

	HBRUSH hbrushFood;
	HBRUSH hbrushBadFood;
	HBRUSH hbrushGoodFood;
	HBRUSH hBrushSnake;
	HBRUSH hOldBrush, hOldBrush2,hOldBrush3;
	...
	PGAME_COORD lpFood, lpBadFood, lpGoodFood, lpBadFood2, lpBadFood3;
	int i, snake_size;
	...
	hbrushBadFood= CreateSolidBrush(COLOR_BADFOOD);
	hbrushGoodFood = CreateSolidBrush(COLOR_GOODFOOD);
	hpen = CreatePen(PS_NULL, 0, RGB(0, 0, 0));  // PEN， PS_NULL表示不可见
	hpen2= CreatePen(PS_NULL, 0, RGB(255, 0, 0));
	hpen3 = CreatePen(PS_NULL, 0, RGB(255, 0, 255));
	...
	*******************************************************************************/
	...
	//画坏的食物
	hOldBrush2 = (HBRUSH)SelectObject(hdcmem, hbrushBadFood);
	hOldPen2 = (HPEN)SelectObject(hdcmem, hpen2);
	lpBadFood = GetBadFood();
	Rectangle(hdcmem,
		lpBadFood->x * CELL_PIXEL + rectBoundary.left,
		lpBadFood->y * CELL_PIXEL + rectBoundary.top,
		(lpBadFood->x + 1) * CELL_PIXEL + rectBoundary.left,
		(lpBadFood->y + 1) * CELL_PIXEL + rectBoundary.top);
	//画坏的食物2
	hOldBrush2 = (HBRUSH)SelectObject(hdcmem, hbrushBadFood);
	hOldPen2 = (HPEN)SelectObject(hdcmem, hpen2);
	lpBadFood2 = GetBadFood2();
	Rectangle(hdcmem,
		lpBadFood2->x* CELL_PIXEL + rectBoundary.left,
		lpBadFood2->y* CELL_PIXEL + rectBoundary.top,
		(lpBadFood2->x + 1)* CELL_PIXEL + rectBoundary.left,
		(lpBadFood2->y + 1)* CELL_PIXEL + rectBoundary.top);
	//画坏的食物3
	hOldBrush2 = (HBRUSH)SelectObject(hdcmem, hbrushBadFood);
	hOldPen2 = (HPEN)SelectObject(hdcmem, hpen2);
	lpBadFood3 = GetBadFood3();
	Rectangle(hdcmem,
		lpBadFood3->x* CELL_PIXEL + rectBoundary.left,
		lpBadFood3->y* CELL_PIXEL + rectBoundary.top,
		(lpBadFood3->x + 1)* CELL_PIXEL + rectBoundary.left,
		(lpBadFood3->y + 1)* CELL_PIXEL + rectBoundary.top);

	//画好的食物
	hOldBrush3 = (HBRUSH)SelectObject(hdcmem, hbrushGoodFood);
	hOldPen3 = (HPEN)SelectObject(hdcmem, hpen3);
	lpGoodFood = GetGoodFood();
	Rectangle(hdcmem,
		lpGoodFood->x* CELL_PIXEL + rectBoundary.left,
		lpGoodFood->y* CELL_PIXEL + rectBoundary.top,
		(lpGoodFood->x + 1)* CELL_PIXEL + rectBoundary.left,
		(lpGoodFood->y + 1)* CELL_PIXEL + rectBoundary.top);
	/*******************************************************************************
	* #############  画蛇  ################
	*
	*******************************************************************************/
	...
	*******************************************************************************/
	...
	DeleteObject(hbrushBadFood);
	DeleteObject(hbrushGoodFood);
