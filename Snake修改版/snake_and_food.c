...
/************* 全局变量 ****************/
// 食物
GAME_COORD food, badfood, goodfood, badfood2, badfood3;
// 蛇（链表）
PLIST snake_list;
// 蛇移动方向
dirction snake_dir;
...
new_badfood:
	...
	// 随机生成坏的食物的坐标。
	badfood.x = rand() % boundary.x;
	badfood.y = rand() % boundary.y;

	// 判断是否和蛇重叠了，否则重新生成坏的食物坐标，直到不重叠。

	for (i = 0; i < size; i++)
	{
		posbody = (PGAME_COORD)ListGetAt(snake_list, i);
		if (CoordEqual(posbody, &(badfood)))
		{
			goto new_badfood;
		}
	}
...
new_goodfood:
	...
	// 随机生成好的食物的坐标。
	goodfood.x = rand() % boundary.x;
	goodfood.y = rand() % boundary.y;

	// 判断是否和蛇重叠了，否则重新生成好的食物坐标，直到不重叠。

	for (i = 0; i < size; i++)
	{
		posbody = (PGAME_COORD)ListGetAt(snake_list, i);
		if (CoordEqual(posbody, &(goodfood)))
		{
			goto new_goodfood;
		}
	}
...
new_badfood2:
	...
	// 随机生成坏的食物的坐标。
	badfood2.x = rand() % boundary.x;
	badfood2.y = rand() % boundary.y;
	...
	{
		posbody = (PGAME_COORD)ListGetAt(snake_list, i);
		if (CoordEqual(posbody, &(badfood2)))
		{
			goto new_badfood2;
		}
	}
...
new_badfood3:
	...
	// 随机生成坏的食物的坐标。
	badfood3.x = rand() % boundary.x;
	badfood3.y = rand() % boundary.y;

	// 判断是否和蛇重叠了，否则重新生成坏的食物坐标3，直到不重叠。

	for (i = 0; i < size; i++)
	{
		posbody = (PGAME_COORD)ListGetAt(snake_list, i);
		if (CoordEqual(posbody, &(badfood3)))
		{
			goto new_badfood3;
		}
	}
...
int SnakeMove()
{
	...
	else if (CoordEqual(posHead, &badfood))
	{
		return SNAKE_EATEN_BADFOOD;
	}
	else if (CoordEqual(posHead, &goodfood))
	{
		return SNAKE_EATEN_GOODFOOD;
	}
